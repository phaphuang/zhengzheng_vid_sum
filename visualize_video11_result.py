...
path = "log/tvsum-split/result_user_summary_split_2.h5"
h5_res = h5py.File(path, 'r')
keys = h5_res.keys()
print(h5_res['video_11'].keys())
...
for key in keys:
    if key == 'video_11':
        ...
        n = len(gtscore)
        plt.title("Dataset=tvsum,canonical  Split=3  Video={}  F-score={:.1f}  XCorr={:.2f}".format(key, fm*100, max_xcorr))
        ...
        lns2 = ax2.plot(range(n), score, color='blue', label="User Summary")
